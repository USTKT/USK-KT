...
class UKT(nn.Module):
    ...
    def base_emb(self, q_data, target):
        q_mean_embed_data = self.mean_q_embed(q_data) # BS, seqlen,  d_model# c_ct
...
def print_cov(mean_tensor,cov_diag_tensor,save_dir):
    import numpy as np
    ...
    plt.scatter(samples[:, 0], samples[:, 1], alpha=0.5, label=f'Sample')

    plt.xlabel('Dimension 1')

    plt.ylabel('Dimension 2')

    plt.title(f'Gaussian Distribution for Sample')
