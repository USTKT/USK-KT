...
def early_fusion(curhs, model, model_name):
    if model_name in ["dkvmn","skvmn"]:
        ...
        p = p.squeeze(-1)
    elif model_name in ["akt", "folibikt","dtransformer","simplekt", "ustkt", "bakt_time", "sparsekt", "akt_vector", "akt_norasch", "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx"]:
...
def effective_fusion(df, model, model_name, fusion_type):
    ...
    hasearly = ["dkvmn","deep_irt", "skvmn", "kqn", "akt", "folibikt", "dtransformer", "simplekt", "ustkt","bakt_time", "sparsekt", "saint", "sakt", "hawkes", "akt_vector", "akt_norasch", "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx", "lpkt"]
    for ui in df:
        # 一题一题处理
        curdf = ui[1]
...
def group_fusion(dmerge, model, model_name, fusion_type, fout):
    ...
    hasearly = ["dkvmn","deep_irt", "skvmn", "kqn", "dtransformer", "akt","folibikt","simplekt","ustkt", "bakt_time", "sparsekt", "saint", "sakt", "hawkes", "akt_vector", "akt_norasch", "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx", "lpkt"]
...
def predict_each_group(dtotal, dcur, dforget, curdforget, is_repeat, qidx, uid, idx, model_name, model, t, end, fout, atkt_pad=False, maxlen=200):
    ...
    for k in range(t, end):
        ...
        if model_name in ["atdkt"]: ## need change!
           ...
           pred = y[0][-1]
        elif model_name in ["simplekt","ustkt", "sparsekt"]:
           if qout != None:
               curq = torch.tensor([[qout.item()]]).to(device)
               qinshft = torch.cat((qin[:,1:], curq), axis=1)
           else:
