...
def cal_loss(model, ys, r, rshft, sm, preloss=[]):
    ...
    elif model_name in ["dkt", "sakt", "saint","hawkes"]:
...
def model_forward(model, data, rel=None):
    ...
    if model_name in ["ustkt"] and model.use_CL != 0:
        return loss,temp
    else:
        return loss
    ...
    for i in range(1, num_epochs + 1):
        ...
        for data in train_loader:
            ...
            if model.model_name in que_type_models:
                ...
                model.train()
            if model.model_name in ["ukt"] and model.use_CL != 0:
                loss,temp = model_forward(model, data)
                temps.append(temp)
            else :
                loss = model_forward(model, data)
            if model.model_name in ["ustkt"] and model.use_CL != 0:
                loss,temp = model_forward(model, data)
                temps.append(temp)
            else :
                loss = model_forward(model, data)
       ...
       ### atkt 有diff， 以下代码导致的
        ...
        if i - best_epoch >= 10:
            break
    if model.model_name in ["ukt","ustkt"]:
        return testauc, testacc, window_testauc, window_testacc, validauc, validacc, best_epoch,best_temps
    else:
