...
class USTKTDataset(Dataset):
    ...
    """
    def __init__(self, file_path, input_type, folds, qtest=False, need_aug=True):
        super(USTKTDataset, self).__init__()
    ...
    def __getitem__(self, index):
        ...
        if "tseqs" in self.dori:
            seqs = self.dori["tseqs"][index][:-1] * mseqs
            shft_seqs = self.dori["tseqs"][index][1:] * mseqs
            dcur["tseqs"] = seqs
            dcur["shft_tseqs"] = shft_seqs

        if self.need_aug:
            rshft_aug = dcur["shft_rseqs"].clone()
            r_aug     = dcur["rseqs"].clone()
            rshft_aug_mask = rshft_aug[mseqs]
            r_aug_mask = r_aug[mseqs]

            if rshft_aug_mask[-1] == 1:
                if r_aug_mask[0] == 1:
                    r_aug[0] = 1 - r_aug[0]
                for i in range(0, len(rshft_aug_mask)):
                    if rshft_aug[0] == 1:
                        rshft_aug[i] = 1 - rshft_aug[i]
            elif rshft_aug_mask[-1] == 0:
                if r_aug_mask[0] == 0:
                    r_aug[0] = 1 - r_aug[0]
                for i in range(0, len(rshft_aug_mask)):
                    if rshft_aug[0] == 0:
    ...
    def __load_data__(self, sequence_path, folds, pad_val=-1):
        ...
        for i, row in df.iterrows():
            ...
            if "timestamps" in row:
                raw_time = [int(_) for _ in row["timestamps"].split(",")]
                base_time = raw_time[0]
                relative_time = [(t - base_time) / 1000.0 for t in raw_time]  
                dori["tseqs"].append(relative_time)
            else:
                dori["tseqs"].append([i for i in range(len(row["questions"].split(",")))])
            if "usetimes" in row:
                dori["utseqs"].append([int(_) for _ in row["usetimes"].split(",")])
                 
            if "uid" in row:
                dori["uid"].append(row["uid"])    
