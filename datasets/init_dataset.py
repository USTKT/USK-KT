...
from .ustkt_dataloader import USTKTDataset
...
def init_test_datasets(data_config, model_name, batch_size, diff_level=None):
    ...
    elif model_name in ["ustkt"]:
        # 1. Base test dataset
        test_dataset = USTKTDataset(os.path.join(data_config["dpath"], data_config["test_file"]),
                        data_config["input_type"], folds=[-1])
        test_loader = DataLoader(test_dataset, batch_size=batch_size)

        # 2. Window test dataset
        test_window_dataset = USTKTDataset(os.path.join(data_config["dpath"], data_config["test_window_file"]),
                        data_config["input_type"], folds=[-1])
        test_window_loader = DataLoader(test_window_dataset, batch_size=batch_size)
        
        # 3. Question test dataset (使用 qtest=True)
        test_question_dataset = USTKTDataset(os.path.join(data_config["dpath"], data_config["test_question_file"]),
                        data_config["input_type"], folds=[-1], qtest=True)
        test_question_loader = DataLoader(test_question_dataset, batch_size=batch_size)

        # 4. Question window test dataset (使用 qtest=True)
        test_question_window_dataset = USTKTDataset(os.path.join(data_config["dpath"], data_config["test_question_window_file"]),
                        data_config["input_type"], folds=[-1], qtest=True)
        test_question_window_loader = DataLoader(test_question_window_dataset, batch_size=batch_size)

        # 确保返回 8 个值 (4个loader + 4个dataset)
        return test_loader, test_window_loader, test_question_loader, test_question_window_loader, \
               test_dataset, test_window_dataset, test_question_dataset, test_question_window_dataset
    # ==========================================================
    else:
...
def init_dataset4train(dataset_name, model_name, data_config, i, batch_size, diff_level=None):
    ...
    if model_name in que_type_models:
        ...
        curvalid = USTKTDataset(os.path.join(data_config["dpath"], data_config["train_valid_file"]), data_config["input_type"], {i})
        curtrain = USTKTDataset(os.path.join(data_config["dpath"], data_config["train_valid_file"]), data_config["input_type"], all_folds - {i})
    else:
